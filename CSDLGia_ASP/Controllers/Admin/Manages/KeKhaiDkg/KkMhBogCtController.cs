...
{
    public class KkMhBogCtController : Controller
    {
        private readonly CSDLGiaDBContext _db;

        public KkMhBogCtController(CSDLGiaDBContext db)
        {
            _db = db;
        }

        [Route("KkMhBogCt/Store")]
        [HttpPost]
        ...
        [Route("KkMhBogCt/Edit")]
        [HttpPost]
        public JsonResult Edit(int Id)
        {
            var model = _db.KkMhBogCt.FirstOrDefault(p => p.Id == Id);
            ...
            if (model != null)
            {
                ...
                result += "<label><b>Tên hàng hóa, dịch vụ*</b></label>";
                result += "<input type='text' id='tendvcu_edit' name='tendvcu_edit' value='" + model.Tenhh + "' class='form-control'/>";
                result += "</div>";
                result += "</div>";
                result += "<div class='col-xl-12'>";
                result += "<div class='form-group fv-plugins-icon-container'>";
                result += "<label><b>Phân loại hàng hóa, dịch vụ</b></label>";
                result += "<textarea type='text' id='qccl_edit' name='qccl_edit' cols='12' rows='3' class='form-control'>" + model.Quycach + "</textarea>";
                result += "</div>";
                result += "</div>";
                ...
                result += "<div class='col-xl-6'>";
                result += "<div class='form-group fv-plugins-icon-container'>";
                result += "<label><b>Giá liền kề*</b></label>";
                ...
                result += "<div class='col-xl-6'>";
                result += "<div class='form-group fv-plugins-icon-container'>";
                result += "<label><b>Giá kê khai*</b></label>";
        ...
        {
            var model = _db.KkMhBogCt.FirstOrDefault(t => t.Id == Id);
            model.Tenhh = Tendvcu;
            model.Plhh = Qccl;
            model.Dvt = Dvt;
            model.Gialk = Gialk;
        ...
        [Route("KkMhBogCt/Delete")]
        [HttpPost]
        public JsonResult Delete(int Id)
        {
            var model = _db.KkMhBogCt.FirstOrDefault(t => t.Id == Id);
            _db.KkMhBogCt.Remove(model);
            _db.SaveChanges();
            string result = GetData(model.Mahs);
            var data = new { status = "success", message = result };
            return Json(data);
        ...
        {
            var model = _db.KkMhBogCt.Where(t => t.Mahs == Mahs).ToList();
            ...
            result += "<th>Phân loại hàng hóa, dịch vụ</th>";
