...
{
    public class KkGiaHplxXdController : Controller
    {
        private readonly CSDLGiaDBContext _db;

        public KkGiaHplxXdController(CSDLGiaDBContext db) => _db = db;

        [Route("XetDuyetGiaHplx")]
        [HttpGet]
        public IActionResult Index(string Madv, string Nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkghplx.giakkxd", "Index"))
                {
                    ...
                    if (getdonvi.Level == "H")
                    {
                        var model = _db.KkGia.Where(t => t.Madv_h == Madv && t.Ngaychuyen_h.Year == int.Parse(Nam) && t.Manghe == "HOCPHILX"
                                                    && (t.Trangthai != "CC" || t.Trangthai != "BTL")).ToList();

                        var model_join = from kk in model
                                         join dn in _db.Company on kk.Madv equals dn.Madv
                                         join dnct in _db.CompanyLvCc.Where(t => t.Manghe == "HOCPHILX") on dn.Mahs equals dnct.Mahs
                                         select new VMKkGia
                                         {
                                             Id = kk.Id,
                                             Tendn = dn.Tendn,
                        ...
                        ViewData["Madv"] = Madv;
                        ViewData["Nam"] = Nam;
                        ViewData["Title"] = "Xét duyệt hồ sơ kê khai giá học phí lái xe";
                        ViewData["MenuLv1"] = "menu_kknygia";
                        ViewData["MenuLv2"] = "menu_kkghplx";
                        ViewData["MenuLv3"] = "menu_giakkhplxxd";
                        return View("Views/Admin/Manages/KeKhaiGia/KkGiaHplx/XetDuyet/Index.cshtml", model_join);
                    }
                    else if (getdonvi.Level == "T")
                    {
                        var model = _db.KkGia.Where(t => t.Madv_t == Madv && t.Ngaychuyen_t.Year == int.Parse(Nam) && t.Manghe == "HOCPHILX"
                                                        && (t.Trangthai != "CC" || t.Trangthai != "BTL")).ToList();
                    ...
                    {
                        var model = _db.KkGia.Where(t => t.Madv_ad == Madv && t.Ngaychuyen_ad.Year == int.Parse(Nam) && t.Manghe == "HOCPHILX"
                                                         && (t.Trangthai != "CC" || t.Trangthai != "BTL")).ToList();
        ...
        public IActionResult TraLai(int id_tralai, string madv_tralai, string Lydo)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkghplx.giakkxd", "Approve"))
                {
                    ...
                    return RedirectToAction("Index", "KkGiaHplxXd", new { Madv = madv_tralai, Nam = model.Ngaynhap.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult XetDuyet(int id_nhanhs, string Sohsnhan, DateTime Ngaynhan)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkghplx.giakkxd", "Approve"))
                {
                    ...
                    return RedirectToAction("Index", "KkGiaHplxXd", new { Madv = model.Macqcq, Nam = model.Ngaynhap.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult CongBo(string mahs_cb, string trangthai_cb)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkghplx.giakkxd", "Approve"))
                {
                    ...
                    return RedirectToAction("Index", "KkGiaHplxXd");
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
