...
{
    public class KkCuocVcHkBcController : Controller
    {
        private readonly CSDLGiaDBContext _db;

        public KkCuocVcHkBcController(CSDLGiaDBContext db)
        {
            _db = db;
        }

        [Route("BaoCaoKkCuocVcHk")]
        [HttpGet]
        public IActionResult Index()
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkcvchk.giakkbc", "Index"))
                {
                    ViewData["Title"] = "Báo cáo tổng hợp kê khai giá cước vận chuyển hành khách";
                    ViewData["MenuLv1"] = "menu_kknygia";
                    ViewData["MenuLv2"] = "menu_kkgvchk";
                    ViewData["MenuLv3"] = "menu_giakkvchkbc";
                    return View("Views/Admin/Manages/KeKhaiGia/KkCuocVcHk/BaoCao/Index.cshtml");
                }
                else
                {
        ...
        public IActionResult BcTongHop(string phanloai, string time, DateTime tungay, DateTime denngay, string thang, string quy, string nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkcvchk.giakk", "Index"))
                {
                    ...
                    ViewData["quy"] = quy;
                    ViewData["nam"] = nam;
                    ViewData["Title"] = "Báo cáo tổng hợp kê khai giá cước vận chuyển hành khác";
                    ViewData["MenuLv1"] = "menu_kknygia";
                    ViewData["MenuLv2"] = "menu_kkgvchk";
                    ViewData["MenuLv3"] = "menu_giakkvchkbc";
                    return View("Views/Admin/Manages/KeKhaiGia/KkCuocVcHk/BaoCao/Bc1.cshtml", model);
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult BcChiTiet(string phanloai, string time, DateTime tungay, DateTime denngay, string thang, string quy, string nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkcvchk.giakkbc", "Index"))
                {
                    ...
                    ViewData["quy"] = quy;
                    ViewData["nam"] = nam;
                    ViewData["ct"] = _db.KkCuocVcHkCt.ToList();
                    ViewData["Title"] = "Báo cáo tổng hợp kê khai giá cước vận chuyển hành khách";
                    ViewData["MenuLv1"] = "menu_kknygia";
                    ViewData["MenuLv2"] = "menu_kkgvchk";
                    ViewData["MenuLv3"] = "menu_giakkvchkbc";
                    return View("Views/Admin/Manages/KeKhaiGia/KkCuocVcHk/BaoCao/Bc2.cshtml", model);
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
