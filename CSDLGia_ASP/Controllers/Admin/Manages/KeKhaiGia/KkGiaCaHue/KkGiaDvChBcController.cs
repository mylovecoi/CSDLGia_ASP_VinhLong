...
{
    public class KkGiaDvChBcController : Controller
    {
        private readonly CSDLGiaDBContext _db;

        public KkGiaDvChBcController(CSDLGiaDBContext db)
        {
            _db = db;
        }

        [Route("BaoCaoKkGiaDvCh")]
        [HttpGet]
        public IActionResult Index()
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgcahue.giakkbc", "Index"))
                {
                    ...
                    ViewData["MenuLv3"] = "menu_giakkcahuebc";
                    return View("Views/Admin/Manages/KeKhaiGia/KkGiaDvCh/BaoCao/Index.cshtml");
                }
                else
                {
        ...
        public IActionResult BcTongHop(string phanloai, string time, DateTime tungay, DateTime denngay, string thang, string quy, string nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgcahue.giakkbc", "Index"))
                {
                    ...
                    ViewData["MenuLv2"] = "menu_kkgcahue";
                    ViewData["MenuLv3"] = "menu_giakkcahuebc";
                    return View("Views/Admin/Manages/KeKhaiGia/KkGiaDvCh/BaoCao/Bc1.cshtml", model);
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult BcChiTiet(string phanloai, string time, DateTime tungay, DateTime denngay, string thang, string quy, string nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgcahue.giakkbc", "Index"))
                {
                    ...
                    ViewData["quy"] = quy;
                    ViewData["nam"] = nam;
                    ViewData["ct"] = _db.KkGiaDvChCt.ToList();
                    ViewData["Title"] = "Báo cáo tổng hợp kê khai giá dịch vụ ca Huế sông Hương";
                    ViewData["MenuLv1"] = "menu_kknygia";
                    ViewData["MenuLv2"] = "menu_kkgcahue";
                    ViewData["MenuLv3"] = "menu_giakkcahuebc";
                    return View("Views/Admin/Manages/KeKhaiGia/KkGiaDvCh/BaoCao/Bc2.cshtml", model);
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
