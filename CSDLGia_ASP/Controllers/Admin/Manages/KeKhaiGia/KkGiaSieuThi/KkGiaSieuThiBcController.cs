...
{
    public class KkGiaSieuThiBcController : Controller
    {
        private readonly CSDLGiaDBContext _db;

        public KkGiaSieuThiBcController(CSDLGiaDBContext db)
        {
            _db = db;
        }

        [Route("BaoCaoKkGiaSieuThi")]
        [HttpGet]
        public IActionResult Index()
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgsieuthi.giakkbc", "Index"))
                {
        ...
        public IActionResult BcTongHop(string phanloai, string time, DateTime tungay, DateTime denngay, string thang, string quy, string nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgsieuthi.giakkbc", "Index"))
                {
                    ...
                    ViewData["MenuLv2"] = "menu_kkgsieuthi";
                    ViewData["MenuLv3"] = "menu_giakkbc";
                    return View("Views/Admin/Manages/KeKhaiGia/KkGiaSieuThi/BaoCao/Bc1.cshtml", model);
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult BcChiTiet(string phanloai, string time, DateTime tungay, DateTime denngay, string thang, string quy, string nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgsieuthi.giakkbc", "Index"))
                {
                    ...
                    ViewData["quy"] = quy;
                    ViewData["nam"] = nam;
                    ViewData["ct"] = _db.KkGiaSieuThiCt.ToList();
                    ViewData["Title"] = "Báo cáo tổng hợp kê khai giá siêu thị";
                    ViewData["MenuLv1"] = "menu_kknygia";
                    ViewData["MenuLv2"] = "menu_kkgsieuthi";
                    ViewData["MenuLv3"] = "menu_giakkbc";
                    return View("Views/Admin/Manages/KeKhaiGia/KkGiaSieuThi/BaoCao/Bc2.cshtml", model);
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
