...
{
    public class KkGiaXmTxdBcController : Controller
    {
        private readonly CSDLGiaDBContext _db;

        public KkGiaXmTxdBcController(CSDLGiaDBContext db)
        {
            _db = db;
        }

        [Route("BaoCaoKkGiaXmTxd")]
        [HttpGet]
        public IActionResult Index()
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgxmtxd.giakkbc", "Index"))
                {
                    ...
                    return View("Views/Admin/Manages/KeKhaiGia/KkGiaXmTxd/BaoCao/Index.cshtml");
                }
                else
                {
        ...
        public IActionResult BcTongHop(string phanloai, string time, DateTime tungay, DateTime denngay, string thang, string quy, string nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgxmtxd.giakkbc", "Index"))
                {
                    ...
                    ViewData["MenuLv2"] = "menu_kkgxmtxd";
                    ViewData["MenuLv3"] = "menu_giakkbc";
                    return View("Views/Admin/Manages/KeKhaiGia/KkGiaXmTxd/BaoCao/Bc1.cshtml", model);
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult BcChiTiet(string phanloai, string time, DateTime tungay, DateTime denngay, string thang, string quy, string nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgxmtxd.giakkbc", "Index"))
                {
                    ...
                    ViewData["quy"] = quy;
                    ViewData["nam"] = nam;
                    ViewData["ct"] = _db.KkGiaXmTxdCt.ToList();
                    ViewData["Title"] = "Báo cáo tổng hợp kê khai giá xi măng thép xây dựng";
                    ViewData["MenuLv1"] = "menu_kknygia";
                    ViewData["MenuLv2"] = "menu_kkgxmtxd";
                    ViewData["MenuLv3"] = "menu_giakkbc";
                    return View("Views/Admin/Manages/KeKhaiGia/KkGiaXmTxd/BaoCao/Bc2.cshtml", model);
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
