...
{
    public class KkGiaXmTxdXdController : Controller
    {
        private readonly CSDLGiaDBContext _db;

        public KkGiaXmTxdXdController(CSDLGiaDBContext db)
        {
            _db = db;
        }

        [Route("XetDuyetGiaXmTxd")]
        [HttpGet]
        public IActionResult Index(string Madv, string Nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgxmtxd.giakkxd", "Index"))
                {
                    ...
                    if (getdonvi.Level == "H")
                    {
                        var model = _db.KkGia.Where(t => t.Madv_h == Madv && t.Ngaychuyen_h.Year == int.Parse(Nam) && t.Manghe == "XMTXD"
                                                    && (t.Trangthai != "CC" || t.Trangthai != "BTL")).ToList();

                        var model_join = from kk in model
                                         join dn in _db.Company on kk.Madv equals dn.Madv
                                         join dnct in _db.CompanyLvCc.Where(t => t.Manghe == "XMTXD") on dn.Mahs equals dnct.Mahs
                                         select new VMKkGia
                                         {
                                             Id = kk.Id,
                                             Tendn = dn.Tendn,
                    ...
                    {
                        var model = _db.KkGia.Where(t => t.Madv_t == Madv && t.Ngaychuyen_t.Year == int.Parse(Nam) && t.Manghe == "XMTXD"
                                                        && (t.Trangthai != "CC" || t.Trangthai != "BTL")).ToList();
                        ...
                        ViewData["Madv"] = Madv;
                        ViewData["Nam"] = Nam;
                        ViewData["Sohsnhan"] = _db.KkGia.Where(t => t.Madv_h == Madv && t.Manghe == "XMTXD")?.Max(t => t.Sohsnhan) ?? "0";
                        ViewData["Title"] = "Xét duyệt hồ sơ kê khai giá xi măng thép xây dựng";
                        ViewData["MenuLv1"] = "menu_kknygia";
                        ViewData["MenuLv2"] = "menu_kkgxmtxd";
                        ViewData["MenuLv3"] = "menu_giakkxd";
                    ...
                    {
                        var model = _db.KkGia.Where(t => t.Madv_ad == Madv && t.Ngaychuyen_ad.Year == int.Parse(Nam) && t.Manghe == "XMTXD"
                                                         && (t.Trangthai != "CC" || t.Trangthai != "BTL")).ToList();
        ...
        public IActionResult TraLai(int id_tralai, string madv_tralai, string Lydo)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgxmtxd.giakkxd", "Approve"))
                {
                    ...
                    return RedirectToAction("Index", "KkGiaXmTxdXd", new { Madv = madv_tralai, Nam = model.Ngaynhap.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult XetDuyet(int id_nhanhs, string Sohsnhan, DateTime Ngaynhan)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgxmtxd.giakkxd", "Approve"))
                {
                    ...
                    return RedirectToAction("Index", "KkGiaXmTxdXd", new { Madv = model.Macqcq, Nam = model.Ngaynhap.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult CongBo(string mahs_cb, string trangthai_cb)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgxmtxd.giakkxd", "Approve"))
                {
                    ...
                    return RedirectToAction("Index", "KkGiaXmTxdXd");
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
