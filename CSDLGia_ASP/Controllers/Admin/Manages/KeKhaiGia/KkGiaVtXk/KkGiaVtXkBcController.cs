...
{
    public class KkGiaVtXkBcController : Controller
    {
        private readonly CSDLGiaDBContext _db;

        public KkGiaVtXkBcController(CSDLGiaDBContext db)
        {
            _db = db;
        }

        [Route("BaoCaoKkGiaVtXk")]
        [HttpGet]
        public IActionResult Index()
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgvtxk.giakkbc", "Index"))
                {
                    ViewData["Title"] = "Báo cáo tổng hợp kê khai giá cước vận tải hành khách bằng ôtô tuyến cố định";
                    ViewData["MenuLv1"] = "menu_kknygia";
                    ViewData["MenuLv2"] = "menu_kkgvtxk";
                    ViewData["MenuLv3"] = "menu_giakkvtxkbc";
                    return View("Views/Admin/Manages/KeKhaiGia/KkGiaVtXk/BaoCao/Index.cshtml");
                }
                else
                {
        ...
        public IActionResult BcTongHop(string phanloai, string time, DateTime tungay, DateTime denngay, string thang, string quy, string nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgvtxk.giakkbc", "Index"))
                {
                    ...
                    ViewData["MenuLv2"] = "menu_kkgvtxk";
                    ViewData["MenuLv3"] = "menu_giakkvtxkbc";
                    return View("Views/Admin/Manages/KeKhaiGia/KkGiaVtXk/BaoCao/Bc1.cshtml", model);
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult BcChiTiet(string phanloai, string time, DateTime tungay, DateTime denngay, string thang, string quy, string nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgvtxk.giakkbc", "Index"))
                {
                    ...
                    ViewData["quy"] = quy;
                    ViewData["nam"] = nam;
                    ViewData["ct"] = _db.KkGiaVtXkCt.ToList();
                    ViewData["Title"] = "Báo cáo tổng hợp kê khai giá cước vận tải hành khách bằng ôtô tuyến cố định";
                    ViewData["MenuLv1"] = "menu_kknygia";
                    ViewData["MenuLv2"] = "menu_kkgvtxk";
                    ViewData["MenuLv3"] = "menu_giakkvtxkbc";
                    return View("Views/Admin/Manages/KeKhaiGia/KkGiaVtXk/BaoCao/Bc2.cshtml", model);
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
