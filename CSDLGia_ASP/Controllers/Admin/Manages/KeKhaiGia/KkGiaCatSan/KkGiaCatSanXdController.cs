...
{
    public class KkGiaCatSanXdController : Controller
    {
        private readonly CSDLGiaDBContext _db;

        public KkGiaCatSanXdController(CSDLGiaDBContext db) => _db = db;

        [Route("XetDuyetGiaCatSan")]
        [HttpGet]
        public IActionResult Index(string Madv, string Nam)
        {
            ...
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgcatsan.giakkxd", "Index"))
                {
                    ...
                    if (getdonvi.Level == "H")
                    {
                        var model = _db.KkGia.Where(t => t.Madv_h == Madv && t.Ngaychuyen_h.Year == int.Parse(Nam) && t.Manghe == "CATSAN"
                                                    && (t.Trangthai != "CC" || t.Trangthai != "BTL")).ToList();

                        var model_join = from kk in model
                                         join dn in _db.Company on kk.Madv equals dn.Madv
                                         join dnct in _db.CompanyLvCc.Where(t => t.Manghe == "CATSAN") on dn.Mahs equals dnct.Mahs
                                         select new VMKkGia
                                         {
                                             Id = kk.Id,
                                             Tendn = dn.Tendn,
                        ...
                        ViewData["MenuLv3"] = "menu_giakkcatsanxd";
                        return View("Views/Admin/Manages/KeKhaiGia/KkGiaCatSan/XetDuyet/Index.cshtml", model_join);
                    }
                    else if (getdonvi.Level == "T")
                    {
                        var model = _db.KkGia.Where(t => t.Madv_t == Madv && t.Ngaychuyen_t.Year == int.Parse(Nam) && t.Manghe == "CATSAN"
                                                        && (t.Trangthai != "CC" || t.Trangthai != "BTL")).ToList();
                    ...
                    {
                        var model = _db.KkGia.Where(t => t.Madv_ad == Madv && t.Ngaychuyen_ad.Year == int.Parse(Nam) && t.Manghe == "CATSAN"
                                                         && (t.Trangthai != "CC" || t.Trangthai != "BTL")).ToList();
        ...
        public IActionResult TraLai(int id_tralai, string madv_tralai, string Lydo)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgcatsan.giakkxd", "Approve"))
                {
                    ...
                    return RedirectToAction("Index", "KkGiaCatSanXd", new { Madv = madv_tralai, Nam = model.Ngaynhap.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult XetDuyet(int id_nhanhs, string Sohsnhan, DateTime Ngaynhan)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgcatsan.giakkxd", "Approve"))
                {
                    ...
                    return RedirectToAction("Index", "KkGiaCatSanXd", new { Madv = model.Macqcq, Nam = model.Ngaynhap.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult CongBo(string mahs_cb, string trangthai_cb)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgcatsan.giakkxd", "Approve"))
                {
                    ...
                    return RedirectToAction("Index", "KkGiaCatSanXd");
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
