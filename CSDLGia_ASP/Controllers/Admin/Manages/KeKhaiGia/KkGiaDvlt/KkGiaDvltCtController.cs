...
{
    public class KkGiaDvltCtController : Controller
    {
        private readonly CSDLGiaDBContext _db;

        public KkGiaDvltCtController(CSDLGiaDBContext db)
        {
            _db = db;
        }

        [Route("KkGiaDvltCt/Store")]
        [HttpPost]
        public JsonResult Store(string Mahs, string Madv, string Macskd, string Tendvcu, string Qccl, string Dvt, double Gialk, double Giakk, string Ghichu)
        {
            var model = new KkGiaDvLtCt
            {
                Mahs = Mahs,
                Madv = Madv,
                Macskd = Macskd,
        ...
        [Route("KkGiaDvltCt/Edit")]
        [HttpPost]
        public JsonResult Edit(int Id)
        {
            var model = _db.KkGiaDvLtCt.FirstOrDefault(p => p.Id == Id);
        ...
        [Route("KkGiaDvltCt/Delete")]
        [HttpPost]
        public JsonResult Delete(int Id)
        {
            var model = _db.KkGiaDvLtCt.FirstOrDefault(t => t.Id == Id);
            _db.KkGiaDvLtCt.Remove(model);
            _db.SaveChanges();
            string result = GetData(model.Mahs);
            var data = new { status = "success", message = result };
            return Json(data);
        ...
        {
            var model = _db.KkGiaDvLtCt.Where(t => t.Mahs == Mahs).ToList();
