...
{
    public class KkGiaVtXtxXdController : Controller
    {
        private readonly CSDLGiaDBContext _db;

        public KkGiaVtXtxXdController(CSDLGiaDBContext db)
        {
            _db = db;
        }

        [Route("XetDuyetGiaVtXtx")]
        [HttpGet]
        public IActionResult Index(string Madv, string Nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgvtxtx.giakkxd", "Index"))
                {
                    ...
                    if (getdonvi.Level == "H")
                    {
                        var model = _db.KkGia.Where(t => t.Madv_h == Madv && t.Ngaychuyen_h.Year == int.Parse(Nam) && t.Manghe == "VTXTX"
                        ...
                        ViewData["Madv"] = Madv;
                        ViewData["Nam"] = Nam;
                        ViewData["Sohsnhan"] = _db.KkGia.Where(t => t.Madv_h == Madv && t.Manghe == "VTXTX")?.Max(t => t.Sohsnhan) ?? "0";
                        ViewData["Title"] = "Xét duyệt hồ sơ kê khai giá cước vận tải hành khách bằng xe taxi";
                        ViewData["MenuLv1"] = "menu_kknygia";
                        ViewData["MenuLv2"] = "menu_kkgvtxtx";
                        ViewData["MenuLv3"] = "menu_giakkvtxtxxd";
                        return View("Views/Admin/Manages/KeKhaiGia/KkGiaVtXtx/XetDuyet/Index.cshtml", model_join);
                    }
                    else if (getdonvi.Level == "T")
                    {
                        var model = _db.KkGia.Where(t => t.Madv_t == Madv && t.Ngaychuyen_t.Year == int.Parse(Nam) && t.Manghe == "VTXTX"
                                                        && (t.Trangthai != "CC" || t.Trangthai != "BTL")).ToList();
                    ...
                    {
                        var model = _db.KkGia.Where(t => t.Madv_ad == Madv && t.Ngaychuyen_ad.Year == int.Parse(Nam) && t.Manghe == "VTXTX"
                                                         && (t.Trangthai != "CC" || t.Trangthai != "BTL")).ToList();
        ...
        public IActionResult TraLai(int id_tralai, string madv_tralai, string Lydo)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgvtxtx.giakkxd", "Approve"))
                {
                    ...
                    return RedirectToAction("Index", "KkGiaVtXtxXd", new { Madv = madv_tralai, Nam = model.Ngaynhap.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult XetDuyet(int id_nhanhs, string Sohsnhan, DateTime Ngaynhan)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgvtxtx.giakkxd", "Approve"))
                {
                    ...
                    return RedirectToAction("Index", "KkGiaVtXtxXd", new { Madv = model.Macqcq, Nam = model.Ngaynhap.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult CongBo(string mahs_cb, string trangthai_cb)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.kknygia.kkgvtxtx.giakkxd", "Approve"))
                {
                    ...
                    return RedirectToAction("Index", "KkGiaVtXtxXd");
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
