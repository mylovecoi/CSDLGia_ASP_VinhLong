        ...
        public IActionResult Index(string Donvi, string Nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.rung.xetduyet", "Index"))
                {
                    ...
                    {
                        var model = _db.GiaRung.Where(t => t.Madv_ad == Donvi && t.Thoidiem_ad.Year == int.Parse(Nam)
                                                         && (t.Trangthai != "HT" || t.Trangthai != "BTL")).ToList();

                        var model_join = from dg in model
                                         join dgct in _db.GiaRungCt on dg.Mahs equals dgct.Mahs
                    ...
                    {
                        var model = _db.GiaRung.Where(t => t.Madv_t == Donvi && t.Thoidiem_t.Year == int.Parse(Nam)
                                                        && (t.Trangthai != "HT" || t.Trangthai != "BTL")).ToList();
        ...
        public IActionResult Complete(string mahs_complete,string Macqcq)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.rung.thongtin", "Approve"))
                {
                    ...
                    _db.SaveChanges();
                    return RedirectToAction("Index", "Giarung", new { model.Mahs, Nam = model.Thoidiem.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
