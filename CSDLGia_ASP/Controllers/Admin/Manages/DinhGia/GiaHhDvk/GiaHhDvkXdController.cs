...
namespace CSDLGia_ASP.Controllers.Admin.Manages.DinhGia.GiaHhDvk
{
    public class GiaHhDvkXdController : Controller
    {
        ...
        public GiaHhDvkXdController(CSDLGiaDBContext db, ITrangThaiHoSoService trangThaiHoSoService)
        {
            _db = db;
        ...
        public IActionResult Index(int Nam, int Thang)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.hhdvk.xd", "Index"))
                {
                    ...
                    IEnumerable<CSDLGia_ASP.Models.Manages.DinhGia.GiaHhDvk> model = _db.GiaHhDvk.Where(t => list_trangthai.Contains(t.Trangthai));
                    ...
                    ViewData["Title"] = "Xét duyệt hồ sơ giá hàng hóa dịch vụ khác";
        ...
        public IActionResult Duyet(string mahs_duyet)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                ...
                {
                    var model = _db.GiaHhDvk.FirstOrDefault(p => p.Mahs == mahs_duyet);
                    ...
                    return RedirectToAction("Index", "GiaHhDvkXd", new { Nam = model.Thoidiem.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult HuyDuyet(string mahs_huyduyet)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.hhdvk.xd", "Approve"))
                {
                    var model = _db.GiaHhDvk.FirstOrDefault(p => p.Mahs == mahs_huyduyet);
        ...
        public IActionResult CongBo(string mahs_cb)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.hhdvk.xd", "Public"))
                {
