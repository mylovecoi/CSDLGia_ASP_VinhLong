...
namespace CSDLGia_ASP.Controllers.Admin.Systems
{
    public class GiaGiaoDichBDSDmController : Controller
    {
        ...
        public GiaGiaoDichBDSDmController(CSDLGiaDBContext db)
        {
            _db = db;
        }

        // Lấy dữ liệu từ bảng GiaGiaoDichBDSDm đổ ra index
        ...
        public IActionResult Index()
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.thuedatnuoc.danhmuc", "Index"))
                {
                    ...
                    ViewData["MenuLv3"] = "menu_dggdbds_dm";
        ...
        // thêm dữ liệu vào bảng GiaGiaoDichBDSDm
        ...
        public JsonResult Store(string Maloaidat, string Loaidat)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "hethong.danhmuc.GiaGiaoDichBDSDm", "Create"))
                {
        ...
        // Xóa bản ghi được chọn trong bảng GiaGiaoDichBDSDm
        ...
        public IActionResult Delete(int id_delete)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "hethong.danhmuc.GiaGiaoDichBDSDm", "Delete"))
                {
                    ...
                    return RedirectToAction("Index", "GiaGiaoDichBDSDm");
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public JsonResult Edit(int Id)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "hethong.danhmuc.GiaGiaoDichBDSDm", "Edit"))
                {
                    ViewData["MenuLv1"] = "menu_dg";
                    ViewData["MenuLv2"] = "menu_dggdbds";
                    ViewData["MenuLv3"] = "menu_dggdbds_tt";
