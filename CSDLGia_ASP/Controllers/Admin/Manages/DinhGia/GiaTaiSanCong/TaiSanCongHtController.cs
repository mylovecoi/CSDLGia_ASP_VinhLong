...
namespace CSDLGia_ASP.Controllers.Admin.Manages.DinhGia.GiaTaiSanCong
{
    public class TaiSanCongHtController : Controller
    {
        ...
        public TaiSanCongHtController(CSDLGiaDBContext db)
        {
            _db = db;
        }
        ...
        public IActionResult Index(string Madv, string Nam)
        {
            ...
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.tsc.giatsc.htg", "Index"))
                {
                    ...
                    {
                        var model = _db.GiaTaiSanCong.Where(t => t.Madv_t == Madv).ToList();
                        if (string.IsNullOrEmpty(Nam))
                        {
                            model = model.ToList();
                        }
                    ...
                    {
                        var model = _db.GiaTaiSanCong.Where(t => t.Madv_h == Madv).ToList();
                        if (string.IsNullOrEmpty(Nam))
                        {
                            model = model.ToList();
                        }
        ...
        public IActionResult HoanThanh(string mahs_complete, string Macqcq)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.tsc.giatsc.htg", "Approve"))
                {
        ...
        public IActionResult ChuyenHoanThanh(string mahs_complete, string Macqcq,string madv_hientai)
        {
            ...
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.tsc.giatsc.htg", "Approve"))
                {
                    ...
                    _db.GiaTaiSanCong.Update(model);
                    _db.SaveChanges();
                    return RedirectToAction("Index", "TaiSanCongHt", new { Madv = madv_hientai, Nam = model.Thoidiem.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult TraLai(int id_tralai, string madv_tralai)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.tsc.giatsc.htg", "Approve"))
                {
                    ...
                    return RedirectToAction("Index", "TaiSanCongHt", new { Madv = madv_tralai, Nam = model.Thoidiem.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
