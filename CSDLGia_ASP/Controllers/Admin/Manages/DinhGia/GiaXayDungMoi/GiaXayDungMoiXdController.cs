...
{
    public class GiaXayDungMoiXdController : Controller
    {
        private readonly CSDLGiaDBContext _db;

        public GiaXayDungMoiXdController(CSDLGiaDBContext db)
        {
            _db = db;
        }

        [Route("GiaXayDungMoi/XetDuyet")]
        [HttpGet]
        public IActionResult Index(string Madv, string Nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.xaydungmoi.xetduyet", "Index"))
                {
                    ...
                    var model = _db.GiaXayDungMoi.ToList();
                    ...
                    if (getdonvi.Level == "H")
                    {
                        ...
                        var model_new = (from kk in model
                                         select new CSDLGia_ASP.Models.Manages.DinhGia.GiaXayDungMoi
                                         {
                                             Id = kk.Id,
                                             Mahs = kk.Mahs,
        ...
        public IActionResult TraLai(int id_tralai, string madv_tralai, string Lydo)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                ...
                {
                    var model = _db.GiaXayDungMoi.FirstOrDefault(t => t.Id == id_tralai);
        ...
        public IActionResult CongBo(string mahs_cb)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.thuetn.xetduyet", "Approve"))
                {
                    var model = _db.GiaXayDungMoi.FirstOrDefault(t => t.Mahs == mahs_cb);
        ...
        public IActionResult TongHop()
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.xaydungmoi.xetduyet", "Index"))
                {
                    ...
                    return View("Views/Admin/Manages/DinhGia/GiaXayDungMoi/DanhSach/Tonghop.cshtml");
        ...
        private static string GetMadvChuyen(string macqcq, CSDLGia_ASP.Models.Manages.DinhGia.GiaXayDungMoi hoso)
        {
            string madv = "";
