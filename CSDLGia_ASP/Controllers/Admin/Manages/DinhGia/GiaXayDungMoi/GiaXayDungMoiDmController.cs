...
{
    public class GiaXayDungMoiDmController : Controller
    {
        private readonly CSDLGiaDBContext _db;
        private readonly IWebHostEnvironment _env;

        public GiaXayDungMoiDmController(CSDLGiaDBContext db, IWebHostEnvironment hostingEnv)
        {
            _db = db;
            _env = hostingEnv;
        }

        [Route("GiaXayDungMoiDmCt")]
        [HttpGet]
        ...
        [Route("GiaXayDungMoiDmCt/Store")]
        [HttpPost]
        public JsonResult Store(string Manhom, string Level, string Cap1, string Cap2, string Cap3, string Cap4, string Cap5, string Dvt, string Tennhom, string Theodoi, string Ghichu)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
        ...
        [Route("GiaXayDungMoiDmCt/Update")]
        [HttpPost]
        ...
        [Route("GiaXayDungMoiDmCt/Delete")]
        [HttpPost]
            ...
            string Cap4, string Cap5, string Dvt, string Ten, int Sheet, int LineStart, int LineStop, IFormFile FormFile)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.thuetn.danhmuc", "Edit"))
                {
                    ...
                    var list_add = new List<GiaXayDungMoiDm>();
                    ...
                    using (var stream = new MemoryStream())
                    {
                        ...
                        using (var package = new ExcelPackage(stream))
                        {
                            ...
                            {
                                list_add.Add(new GiaXayDungMoiDm
                                {
                                    Manhom = Manhom,
                                    Theodoi = "TD",
                                    Created_at = DateTime.Now,
                    ...
                    _db.GiaXayDungMoiDm.AddRange(list_add);
                    _db.SaveChanges();
