...
namespace CSDLGia_ASP.Controllers.Admin.Manages.DinhGia.Giathuemuanhaxh
{
    public class GiathuemuanhaxhHtController : Controller
    {
        ...
        public IActionResult Index(string Madv, string Nam)
        {
            ...
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.thuemuanhaxh.xetduyet", "Index"))
                {
                    ...
                    {
                        var model = _db.GiaThueMuaNhaXh.Where(t => t.Madv_ad == Madv).ToList();
                    ...
                    {
                        var model = _db.GiaThueMuaNhaXh.Where(t => t.Madv_t == Madv).ToList();
                        if (string.IsNullOrEmpty(Nam))
                        {
                            model = model.ToList();
                        }
                    ...
                    {
                        var model = _db.GiaThueMuaNhaXh.Where(t => t.Madv_h == Madv).ToList();
                        if (string.IsNullOrEmpty(Nam))
                        {
                            model = model.ToList();
                        }
        ...
        public IActionResult HoanThanh(string mahs_complete, string Macqcq)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.thuemuanhaxh.xetduyet", "Approve"))
                {
        ...
        public IActionResult ChuyenHoanThanh(string mahs_complete, string Macqcq, string madv_hientai)
        {
            ...
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.tmnxh.xetduyet", "Approve"))
                {
                    var model = _db.GiaThueMuaNhaXh.FirstOrDefault(p => p.Mahs == mahs_complete);
                    ...
                    _db.GiaThueMuaNhaXh.Update(model);
                    _db.SaveChanges();
                    return RedirectToAction("Index", "GiathuemuanhaxhHt", new { Madv = madv_hientai, Nam = model.Thoidiem.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
