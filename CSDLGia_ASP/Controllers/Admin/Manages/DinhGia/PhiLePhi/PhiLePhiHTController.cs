    ...
    public class PhiLePhiHTController : Controller
    {
        ...
        public IActionResult Index(string Madv, string Nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.lephi.xetduyet", "Index"))
                {
                    ...
                    var model = _db.PhiLePhi.ToList();

                    if (getdonvi.Level == "ADMIN")
                    {
                        model = model.Where(t => t.Macqcq == Madv || t.Macqcq_t == Madv || t.Macqcq_h == Madv).ToList();
                    ...
                    {
                        item.Madiaban = getdonvi.MaDiaBan;
                        item.Level = getdonvi.Level;
                    }                   
                    ...
                    return View("Views/Admin/Manages/DinhGia/PhiLePhi/HoanThanh/Index.cshtml", model);
                }
                else
                {
        ...
        public IActionResult HoanThanh(string mahs_complete, string Macqcq)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                ...
                {
                    var model = _db.PhiLePhi.FirstOrDefault(p => p.Mahs == mahs_complete);
                    ...
                    _db.SaveChanges();
                    return RedirectToAction("Index", "PhiLePhiHT", new { Madv = model.Madv, Nam = model.Thoidiem.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult ChuyenHoanThanh(string mahs_complete, string Macqcq, string madv_hientai)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.lp.gialp.ttg", "Approve"))
                {
                    ...
                    _db.PhiLePhi.Update(model);
                    _db.SaveChanges();
                    return RedirectToAction("Index", "PhiLePhiHT", new { Madv = madv_hientai, Nam = model.Thoidiem.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
