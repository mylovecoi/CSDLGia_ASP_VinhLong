    ...
    public class GiaoDucDaoTaoCtController : Controller
    {
        ...
        public JsonResult Edit(int Id)
        {
            ...
            if (model != null)
            {
                ...
                result += "<input type='text' id='tt1_edit' name='tt1_edit'  value='" + Helpers.ConvertDbToStr(model.Giathanhthi1) + "' class='form-control money-decimal-mask' style='font-weight: bold' />";
                result += "</div></div>";
                result += "<div class='col-xl-4'>";
                result += "<div class='form-group fv-plugins-icon-container'>";
                result += "<label><b>Khu vực nông thôn 1</b></label>";
                result += "<input type='text' id='nt1_edit' name='nt1_edit'  value='" + Helpers.ConvertDbToStr(model.Gianongthon1) + "' class='form-control money-decimal-mask' style='font-weight: bold' />";
                result += "</div> </div>";
                result += "<div class='col-xl-4'>";
                result += "<div class='form-group fv-plugins-icon-container'>";
                result += "<label><b>Khu vực miền núi 1</b></label>";
                result += "<input type = 'text' id='mn1_edit' name='mn1_edit'  value='" + Helpers.ConvertDbToStr(model.Giamiennui1) + "' class='form-control money-decimal-mask' style='font-weight: bold' />";
                result += "</div></div></div>";
                ...
                if (data.Any())
                {
                    ...
                    if (data.Any())
                    {
                        foreach (var item in data.OrderBy(t => t.Id))
                        {
                            ...
                            result += "<td style='text-align:right'>" + Helpers.ConvertDbToStr(item.Giathanhthi1) + "</td>";
                            result += "<td style='text-align:right'>" + Helpers.ConvertDbToStr(item.Gianongthon1) + "</td>";
                            result += "<td style='text-align:right'>" + Helpers.ConvertDbToStr(item.Giamiennui1) + "</td>";
                            result += "<td>";
