...
{
    public class GiaThueTaiNguyenNhomController : Controller
    {
        private readonly CSDLGiaDBContext _db;

        public GiaThueTaiNguyenNhomController(CSDLGiaDBContext db)
        {
            _db = db;
        }

        [Route("GiaThueTaiNguyenDm")]
        [HttpGet]
        public IActionResult Index()
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.thuetn.danhmuc", "Index"))
                {
                    ...
                    ViewData["maKetNoiAPI"] = "giathuetainguyendm";
                    return View("Views/Admin/Manages/DinhGia/GiaThueTaiNguyen/DanhMuc/Nhom/Index.cshtml", model);
                }
                else
                {
        ...
        [Route("GiaThueTaiNguyenDm/Store")]
        [HttpPost]
        public JsonResult Store(string Manhom, string Tennhom, string Theodoi)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.thuetn.danhmuc", "Create"))
                {
                    ...
                    _db.GiaThueTaiNguyenNhom.Add(request);
                    _db.SaveChanges();
        ...
        public JsonResult Edit(int Id)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                ...
                {
                    var model = _db.GiaThueTaiNguyenNhom.FirstOrDefault(p => p.Id == Id);
                    if (model != null)
                    {
        ...
        [Route("GiaThueTaiNguyenDm/Delete")]
        [HttpPost]
        public IActionResult Delete(int id_delete)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                ...
                {
                    var model = _db.GiaThueTaiNguyenNhom.FirstOrDefault(p => p.Id == id_delete);
                    _db.GiaThueTaiNguyenNhom.Remove(model);
                    _db.SaveChanges();

                    var model_ct = _db.GiaThueTaiNguyenDm.Where(p => p.Manhom == model.Manhom).ToList();
                    _db.GiaThueTaiNguyenDm.RemoveRange(model_ct);
                    _db.SaveChanges();

                    return RedirectToAction("Index", "GiaThueTaiNguyenNhom");
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
