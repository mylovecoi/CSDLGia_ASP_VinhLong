...
{
    public class GianuocshHtController : Controller
    {
        private readonly CSDLGiaDBContext _db;

        public GianuocshHtController(CSDLGiaDBContext db)
        {
            _db = db;
        }
        
        [Route("HoanThanhDinhGiaNuocSh")]
        [HttpGet]
        public IActionResult Index(string Donvi, string Nam)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.nuocsh.xetduyet", "Index"))
                {
                    ...
                    {
                        var model = _db.GiaNuocSh.Where(t => t.Madv_ad == Donvi && t.Thoidiem_ad.Year == int.Parse(Nam)
                                                         && (t.Trangthai != "HT" || t.Trangthai != "BTL")).ToList();

                        var model_join = from dg in model
                                         join dgct in _db.GiaNuocShCt on dg.Mahs equals dgct.Mahs
                                         select new VMDinhGiaNuocSh
                                         {
                                             Id = dg.Id,
                                             //Macqcq = Donvi,
                                             Mahs = dg.Mahs,
                    ...
                    {
                        var model = _db.GiaNuocSh.Where(t => t.Madv_t == Donvi && t.Thoidiem_t.Year == int.Parse(Nam)
                                                        && (t.Trangthai != "HT" || t.Trangthai != "BTL")).ToList();
        ...
        public IActionResult Complete(string mahs_complete,string Macqcq)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.nuocsh.thongtin", "Approve"))
                {
                    ...
                    _db.SaveChanges();
                    return RedirectToAction("Index", "Gianuocsh", new { model.Mahs, Nam = model.Thoidiem.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult TraLai(int id_tralai, string madv_tralai)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.nuocsh.thongtin", "Approve"))
                {
                    ...
                    return RedirectToAction("Index", "GianuocshHt", new { Madv = madv_tralai, Nam = model.Thoidiem.Year });
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
        ...
        public IActionResult CongBo(string mahs_cb, string trangthai_cb)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                if (Helpers.CheckPermission(HttpContext.Session, "csdlmucgiahhdv.dinhgia.nuocsh.xetduyet", "Approve"))
                {
                    ...
                    return RedirectToAction("Index", "GianuocshHt");
                }
                else
                {
                    ViewData["Messages"] = "Bạn không có quyền truy cập vào chức năng này!";
