...
namespace CSDLGia_ASP.Controllers.Admin.Systems
{
    public class DsPhanQuyenController : Controller
    {
        ...
        public JsonResult Edit(int id)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                ...
                if (model != null)
                {
                    ...
                    result += "<input id='Id_edit' name='Id_edit' value='" + model.Id + "'>";
                    result += "</div>";
                    result += "</div>";
        ...
        [Route("Permissions/Edit/Single")]
        [HttpPost]
        public JsonResult EditSingle(int id)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                var model = _db.Permissions.Find(id);
        ...
        [Route("Permissions/Update/Single")]
        [HttpPost]
        public JsonResult UpdateSingle(string Username, string Roles, bool Index, bool Create, bool Edit, bool Delete, bool Approve,
                                bool Public, string Status, int Id)
        {
            if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
            {
                ...
                string result = this.GetDataPermissionSingle(Username);
                var data = new { status = "success", message = result };
                return Json(data);
            }
            else
        ...
        public string GetDataPermissionSingle(string Username)
        {
            var model = _db.Permissions.Where(t => t.Username == Username);
            int record_id = 1;
            string result = "<div class='card-body' id='thongtins_permission'>";
            result += "<table class='table table-striped table-bordered table-hover' id='sample_3'>";
            result += "<thead>";
            result += "<tr style='text-align:center'>";
            result += "<th width='2%'>#</th>";
            ...
            if (model != null)
            {
                foreach (var item in model.Where(t => t.Level == 0).OrderBy(t => t.Sttsx))
                {
                    ...
                    result += "</td>";
                    result += "<td>";
                    result += "<button type='button' onclick='editIdS(`" + item.Id + "`)' data-target='#Edit_Modal' data-toggle='modal'";
                    result += " class='btn btn-sm btn-clean btn-icon' title='Chỉnh sửa'><i class='icon-lg la la-edit text-primary'></i></button>";
                    result += "</td>";
                    result += "</tr>";
                    if (item.Index == true)
                    {
                        foreach (var item1 in model.Where(t => t.Level == 1 && t.Magoc == item.Roles).OrderBy(t => t.Sttsx))
                        {
                            ...
                            result += "</td>";
                            result += "<td>";
                            result += "<button type='button' onclick='editIdS(`" + item1.Id + "`)' data-target='#Edit_Modal' data-toggle='modal'";
                            result += " class='btn btn-sm btn-clean btn-icon' title='Chỉnh sửa'><i class='icon-lg la la-edit text-primary'></i></button>";
                            result += "</td>";
                            result += "</tr>";
                            if (item1.Index == true)
                            {
                                foreach (var item2 in model.Where(t => t.Level == 2 && t.Magoc == item1.Roles).OrderBy(t => t.Sttsx))
                                {
                                    ...
                                    result += "</td>";
                                    result += "<td>";
                                    result += "<button type='button' onclick='editIdS(`" + item2.Id + "`)' data-target='#Edit_Modal' data-toggle='modal'";
                                    result += " class='btn btn-sm btn-clean btn-icon' title='Chỉnh sửa'><i class='icon-lg la la-edit text-primary'></i></button>";
                                    result += "</td>";
                                    result += "</tr>";
                                    if (item2.Index == true)
                                    {
                                        foreach (var item3 in model.Where(t => t.Level == 3 && t.Magoc == item2.Roles).OrderBy(t => t.Sttsx))
                                        {
                                            ...
                                            result += "</td>";
                                            result += "<td>";
                                            result += "<button type='button' onclick='editIdS(`" + item3.Id + "`)' data-target='#Edit_Modal' data-toggle='modal'";
                                            result += " class='btn btn-sm btn-clean btn-icon' title='Chỉnh sửa'><i class='icon-lg la la-edit text-primary'></i></button>";
                                            result += "</td>";
                                            result += "</tr>";
                                        }
        ...
        //[HttpPost]
        //public JsonResult UpdateSinglePermission(string Username, string Roles, bool Index, bool Create, bool Edit, bool Delete, bool Approve,
        //                        bool Public, string Status, int Id)
        //{
        //    if (!string.IsNullOrEmpty(HttpContext.Session.GetString("SsAdmin")))
        //    {
